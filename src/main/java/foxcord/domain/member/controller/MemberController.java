...
public class MemberController {
    ...
    @GetMapping
    public ResponseEntity<MemberResponse> findMemberInfo(@CookieValue(value = "JWT", required = false) Cookie cookie) {
        MemberResponse memberResponseDto = memberService.findMemberInfo(cookie);
        return ResponseEntity.ok(memberResponseDto);
    }

    @PatchMapping
    public ResponseEntity<Void> updateMemberInfo(@CookieValue(value = "JWT", required = false) Cookie cookie, @RequestBody MemberUpdateRequest memberUpdateRequest) {
        memberService.updateMemberInfo(cookie, memberUpdateRequest);
        return ResponseEntity.ok().build();
    }

    @PatchMapping("/password")
    public ResponseEntity<Void> updateMemberPassword(@CookieValue(value = "JWT", required = false) Cookie cookie, @RequestBody MemberUpdatePasswordRequest memberUpdatePasswordRequest) {
